const app = {
    ...
    calculateTotal() {
        ...
        const ventasTPV = parseFloat(document.getElementById('ventas-efectivo').value) || 0;
        
        const diff = totalRealCaja - (fondoAyer + ventasTPV);
    ...
    shareWhatsApp() {
        ...
        const visa = document.getElementById('resumen-visa').value;
        const efectivo = document.getElementById('resumen-efectivo').value;
        ...
        const text = `*CIERRE DE CAJA (${fecha}) v2.2* 📊%0A%0A` +
                     ...
                     `• Diferencia Datafono: ${diffVisa} ${statusVisa}%0A%0A` +
    ...
    saveClosing() {
        const total = document.getElementById('total-cash-combined').value;
        this.closings.unshift({ date: new Date().toISOString(), totalA: parseFloat(total) });
    ...
    renderHistory() {
        ...
        list.innerHTML = this.closings.length ? this.closings.map(c => `
